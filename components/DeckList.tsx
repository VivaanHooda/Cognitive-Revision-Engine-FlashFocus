...
import { Play, Plus, Brain, Trash2, Wand2, Sparkles, CheckSquare, Square, Loader2, ChevronRight, Layers, Clock, Star } from 'lucide-react';
import { generateCurriculum, generateDeckFromTopic, SubtopicSuggestion } from '@/lib/geminiService';
...
export const DeckList: React.FC<DeckListProps> = ({ decks, onSelectDeck, onAddDeck, onDeleteDeck, apiKey, setApiKey, userId }) => {
  ...
  const handleAnalyzeTopic = async () => {
    ...
    setError(null);
    try {
      const suggestions = await generateCurriculum(topicInput, apiKey);
  ...
  const handleGenerateDecks = async () => {
    ...
    for (const index of selectedIndices) {
      ...
      setProgressLog(prev => [...prev, `Generating content for: ${subtopic.title}...`]);
      
      try {
        const data = await generateDeckFromTopic(subtopic.title, topicInput, apiKey);
  ...
  const getDeckStats = (deck: Deck) => {
    ...
    const dueCards = deck.cards.filter(c => c.status !== 'new' && (!c.dueDate || c.dueDate <= now)).length;
    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-500 mt-1">Manage your learning paths and decks</p>
        </div>
        <button
      ...
      {Object.keys(groupedDecks).length === 0 && (
        <div className="text-center py-20 bg-white rounded-2xl border border-dashed border-gray-300">
          ...
          <p className="text-gray-500 mb-4">Start by creating a new AI-powered study topic.</p>
        </div>
      )}
        ...
        {Object.entries(groupedDecks).map(([topic, topicDecks]) => (
          <div key={topic} className="animate-fade-in-up">
            <div className="flex items-center gap-2 mb-4 border-b border-gray-100 pb-2">
              ...
              <h2 className="text-lg font-bold text-gray-700 uppercase tracking-wide">{topic}</h2>
            </div>
              ...
              {topicDecks.map((deck) => {
                ...
                return (
                  ...
                  >
                    ...
                    <h3 className="text-lg font-bold text-gray-900 mb-2 leading-tight">{deck.title}</h3>
                    <p className="text-gray-500 text-sm mb-6 line-clamp-3 flex-grow">{deck.description}</p>

                    <div className="flex gap-4 mb-4">
                        <div className="flex items-center gap-1.5 text-sm font-medium text-green-600 bg-green-50 px-2 py-1 rounded-md" title="Cards Due Review">
                            <Clock size={14} />
                            <span>{dueCards} Due</span>
                        </div>
                        <div className="flex items-center gap-1.5 text-sm font-medium text-blue-600 bg-blue-50 px-2 py-1 rounded-md" title="New Cards">
                            <Star size={14} />
          ...
          <div className="bg-white rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="px-6 py-5 border-b border-gray-100 bg-gray-50/50 flex justify-between items-center">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-xl font-bold text-gray-900">AI Course Creator</h2>
                  <p className="text-xs text-gray-500">From topic to full curriculum in seconds</p>
                </div>
              </div>
              <button onClick={resetModal} className="text-gray-400 hover:text-gray-600 p-2">✕</button>
            </div>
            
            <div className="p-6 overflow-y-auto flex-grow">
              <div className="mb-6">
                 <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Gemini API Key</label>
                 <input
                  type="password"
                  value={apiKey}
              ...
              {wizardStep === 'INPUT' && (
                <div className="animate-fade-in space-y-4">
                  <div>
                    <label className="block text-lg font-medium text-gray-900 mb-2">What do you want to master?</label>
                    <input
                      type="text"
                      value={topicInput}
                      ...
                      autoFocus
                    />
                    <p className="text-sm text-gray-500 mt-2">The AI will break this topic down into logical sub-modules for you.</p>
                  </div>
              ...
              {wizardStep === 'CURRICULUM' && (
                <div className="animate-fade-in space-y-4">
                  <div className="flex justify-between items-center">
                    <h3 className="font-semibold text-gray-900">Proposed Curriculum</h3>
                    <span className="text-xs font-medium text-indigo-600 bg-indigo-50 px-2 py-1 rounded-full">{selectedSubtopics.size} selected</span>
                  </div>
                  <div className="space-y-3 max-h-80 overflow-y-auto pr-2">
                    {subtopics.map((sub, idx) => {
                      const isSelected = selectedSubtopics.has(idx);
                      return (
                        <div key={idx} onClick={() => toggleSubtopic(idx)} className={`p-4 rounded-xl border-2 cursor-pointer transition-all flex items-start gap-3 ${isSelected ? 'border-indigo-500 bg-indigo-50/50' : 'border-gray-100 hover:border-gray-200 hover:bg-gray-50'}`}>
                          <div className={`mt-1 ${isSelected ? 'text-indigo-600' : 'text-gray-300'}`}>{isSelected ? <CheckSquare size={20} /> : <Square size={20} />}</div>
                          <div>
                            <h4 className={`font-bold ${isSelected ? 'text-indigo-900' : 'text-gray-600'}`}>{sub.title}</h4>
                ...
                <div className="animate-fade-in space-y-6 text-center py-8">
                  {isLoading ? (
                     <div className="flex flex-col items-center">
                        ...
                        <h3 className="text-xl font-bold text-gray-900">Crafting your decks...</h3>
                     </div>
                  ) : (
                  ...
                  <div className="bg-gray-900 rounded-lg p-4 text-left max-h-48 overflow-y-auto font-mono text-xs text-green-400">
                    {progressLog.map((log, i) => <div key={i} className="mb-1 opacity-90">{`> ${log}`}</div>)}
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="p-6 border-t border-gray-100 bg-gray-50 flex justify-end gap-3">
              {wizardStep === 'INPUT' && (
                <button
                  ...
                  disabled={isLoading || !topicInput || !apiKey}
                  className={`px-6 py-3 rounded-xl font-bold text-white flex items-center gap-2 transition-all ${isLoading || !topicInput || !apiKey ? 'bg-gray-300 cursor-not-allowed' : 'bg-indigo-600 hover:bg-indigo-700 shadow-lg hover:shadow-indigo-500/25'}`}
                >
              ...
              {wizardStep === 'CURRICULUM' && (
                <>
                  <button onClick={() => setWizardStep('INPUT')} className="px-4 py-3 rounded-xl font-semibold text-gray-600 hover:bg-gray-200 transition-colors">Back</button>
                  <button
                    onClick={handleGenerateDecks}
                    disabled={selectedSubtopics.size === 0}
                    className={`px-6 py-3 rounded-xl font-bold text-white flex items-center gap-2 transition-all ${selectedSubtopics.size === 0 ? 'bg-gray-300 cursor-not-allowed' : 'bg-indigo-600 hover:bg-indigo-700 shadow-lg hover:shadow-indigo-500/25'}`}
                  >
                    Generate {selectedSubtopics.size} Decks <ChevronRight size={20} />
                  </button>
                </>
              )}
            </div>
