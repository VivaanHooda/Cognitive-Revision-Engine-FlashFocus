...
/**
 * Server-side Anki / SM-2 Algorithm Implementation
 */
...
export const getNextReviewText = (
  interval: number,
  grade: StudyGrade
): string => {
  ...
  if (grade === "good")
    nextI = interval === 0 ? 1 : interval === 1 ? 6 : Math.ceil(interval * 2.5);
  if (grade === "easy")
    nextI = interval === 0 ? 4 : Math.ceil(interval * 2.5 * 1.3);
